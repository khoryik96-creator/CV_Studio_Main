# CV Studio v24.6.221 → Phase 3 Handover

Source release: private owner/source v24.6.221

Completed phase: Phase 2B — browser-backed durable records and settings,
including the corrective review closure
...
the v24.6.221 corrective QA report, the v24.6.220 Phase 2B QA report and verify
the v24.6.221 archive against its adjacent SHA-256 sidecar before changing
production code.
...
- Fully normalize every record array before import/replacement. Never permit a
  record that cannot be stored safely to reach a tombstoning transaction.
...
- Stale imports never overwrite an existing row or tombstone; same-page races
  retain only records/keys actually mutated after hydration began.
...
1. Verify the v24.6.221 owner ZIP and fresh extraction against the adjacent
   SHA-256 and verification sidecars.
2. Verify Git is clean and based on the completed v24.6.221 release commit.
3. Read `AGENTS.md`, `ROADMAP.md`, `PHASE_STATUS.md`, `IMPLEMENT.md`, this
   handover, the v24.6.221/v24.6.220 QA reports and historical Phase 2A reports.
...
5. Re-prove migration idempotency, interruption rollback/restart, corruption
   recovery, tombstones, rejected-replacement preservation and legacy bytes.
...
v24.6.221 has genuine Windows source execution and controlled local fixtures,
