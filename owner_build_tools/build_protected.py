...
def detect_target(value: str) -> str:
    ...
    raise RuntimeError(
        "CV Studio v24.6.241 is a Windows-x64-only release. "
...
def validate_target_host(target: str) -> None:
    ...
    if target != RELEASE_TARGET:
        raise RuntimeError(
            "CV Studio v24.6.241 may produce only a Windows-x64 artifact."
        )
    if system!="windows" or machine not in {"amd64","x86_64"}:
        raise RuntimeError(
            "CV Studio v24.6.241 Windows-x64 packages must be built on "
            "native Windows x64."
        )
...
def compile_native(root: Path, work: Path, target: str, source_entry: Path | None = None) -> tuple[Path,Path]:
    if target != RELEASE_TARGET:
        raise RuntimeError("v24.6.241 native compilation is Windows-x64-only.")
...
def patch_launchers(root: Path,target: str) -> None:
    ...
    if target != RELEASE_TARGET:
        raise RuntimeError("v24.6.241 launcher packaging is Windows-x64-only.")
    if target=="windows-x64":
        for rel in ("START_HIDDEN.vbs", "WATCHDOG.vbs"):
            raw=(root/rel).read_bytes()
            if raw.startswith(b"\xef\xbb\xbf"):
...
def prune_target_incompatible_launchers(root: Path, target: str) -> None:
    ...
    if target != RELEASE_TARGET:
        raise RuntimeError("v24.6.241 artifact pruning is Windows-x64-only.")
    if target == "windows-x64":
...
def build_package(source: Path,work: Path,out_dir: Path,target: str,dist: Path,nuitka_report: Path,
    ...
    if target != RELEASE_TARGET:
        raise RuntimeError("v24.6.241 protected packaging is Windows-x64-only.")
...
def smoke_test(package: Path,source: Path,output: Path,target: str,timeout_seconds: int = 240) -> dict:
    if target != RELEASE_TARGET:
        raise RuntimeError("v24.6.241 protected smoke testing is Windows-x64-only.")
