...
# CV Studio v24.6.237 — Phase 6 Handover

Date: 26 July 2026

Handover source: completed private owner/source JobAdder esc2 corrective release

Phase 6 status: inactive; explicit owner authorization is required

## Completed baseline

CV Studio v24.6.237 completes Phases 1, 2A, 2B, 3, 4, 5A and 5B and includes
the authorized narrow post-5B JobAdder frontend correction.

The candidate-not-found dialog and formatted-CV upload queue now use the
established global `esc()` helper for email, filename and status text. The
upload renderer's local ID variable no longer shadows `esc()`. The valid local
`esc2` helpers inside `renderAnonJDCard()` and `renderCompanyCard()` remain
unchanged.

A source-scope regression requires every `esc2` definition and call to remain
inside those two renderers. No global `esc2` alias was added.

## Immutable corrective contracts

Any later work must preserve unless separately authorized:

- the candidate-not-found JobAdder dialog renders without a scope error and
  HTML-escapes the displayed email;
- matched and unmatched JobAdder upload-queue entries render without a scope
  error;
- upload filenames and status text remain HTML-escaped;
...
1. verify the v24.6.237 owner ZIP, checksum, verification sidecar and exact
   `source_commit`;
2. freshly extract it and compare every tracked Git blob;
3. read this handover, the v24.6.237 corrective QA report, the v24.6.236 Phase
...
6. run all four frontend fixtures and 24 live smoke assertions;
...
- `cv_studio_v24_6_237_jobadder_esc2_corrective_owner_source.zip`;
