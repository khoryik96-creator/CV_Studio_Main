...
- PR #189 merged as `2c216c6`; PR #185 merged as `b84c36f`. Neither is an
  ...
  v24.6.411 through v24.6.414 on `claude/ai-crawler-search-refinement`, the AI
  Crawler blank-field review queue, the save that writes a reviewed tag back into
  JobAdder, and the two correctives that make both actually work safely.
...
- Current stop: the source-ownership run is merged through v24.6.410 (`960a086`, PR #212)
  ...
  review queue, v24.6.412 adds the save that writes a reviewed tag into the
  JobAdder custom field, and v24.6.413 and v24.6.414 correct both: as first
  written the queue never ran, the save could clear unrelated custom fields, and
  the queue then offered candidates whose fields were not actually blank. All four
  await owner testing, and the write path in particular is unverified against a
  live JobAdder tenant. No release or native
...
## v24.6.383 unused-helper cleanup (merged in PR #195)

- Removes only four unreferenced Python definitions. All other function bodies
  remain unchanged, including the v24.6.382 fixes merged in PR #194.
- Integrated validation: 1117 passed, 4 skipped, 128 subtests; all 23 frontend
  groups; 24 live source-smoke assertions; protected-source/dependency preflight
  and repository consistency passed. No native protected compilation.

## v24.6.382 async-handler corrective (merged in PR #194)

- Owner requested fixes from Claude's audit. Guard OneNote, Outlook and CV
  download entry points, preserve failed-disconnect state and close reserved
  tabs on uncertain test-draft responses. Never replay uploads or drafts.
- Queue skip flags are released on interrupted one-item profile creation.
- No CV-formatting, route, schema, dependency or protected-build changes.
- Four unused Python helpers are tracked separately from behavior changes.
...
- Removed rows leave Download All; processing rows cannot be removed. Mixed
  Format/Blind outputs save to their respective configured destinations.
  Download All snapshots its outputs and reports actual save outcomes.
- Owl hides copy/export while regenerating and never exports loading/error DOM
  text. A successful retry restores normal export readiness.
- Complete mocked workflow regressions cover duplicate names, successive runs,
  partial failures, removals, mixed destinations and Owl failure/retry.
- No routes, schemas, dependencies, CV-formatting content, paid-call counts or
  protected-build boundaries changed. No native protected compilation.
- Validation: 1117 Python tests passed, 4 skipped, 128 subtests; all 22 frontend
  groups; 24 live source-smoke assertions; tracked Python/JavaScript/PowerShell
  syntax, repository consistency and Windows protected-source preflight passed.
  The new duplicate-filename regression fails against old master as expected.

## v24.6.380 output-folder shortcuts (merged, PR #192)
...
## v24.6.379 skill-casing corrective (merged with v24.6.380)
...
- The deterministic anonymized-summary safety pass preserves dotted degree
  ...
  recognizes compact, spaced and lowercase Malaysian `A/P` / `A/L` lineage
  spellings, and masks employers in unparenthesized or role-bearing single-year
  work-history rows.
- No route, storage schema, dependency, credential, paid-call-count,
  CV-formatting or protected-package boundary changes. PR #185 remains separate
  and untouched. Local validation: 1060 tests passed, 4 skipped and 128
  ...
  Tesseract/adm-zip preflight.

## v24.6.370 anonymization edge-case corrective (merged through PR #188)
