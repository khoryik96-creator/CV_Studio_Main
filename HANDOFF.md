...
| Remaining `_ja_*` **service** clusters (`_ja_activity` network probes, `_ja_candidate`, `_ja_spa` bridge) | ~3,830–4,560 | service (needs client/request injection) | Einstein/Kingg | network-performing OAuth GET/POST + SPA-bridge helpers; the jobadder_read/write + SalaryAiCacheService injection pattern applies. Not pure closures |
