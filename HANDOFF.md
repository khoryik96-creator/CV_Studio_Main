...
| JobAdder `_ja_activity` diagnostic/result helpers → **`cvstudio_ja_activity.py`** | ~4,350–4,660 | pure closure (slice 1) | Kingg | ✅ merged to branch (Kingg, 2026-08-09) — 5 pure helpers (diagnostic header redaction / network-error label / read summary + the two activity-id parsers) with new characterization tests. The OAuth GET/POST **network probes** + routes stay in the shell for a later service slice |
| `_ja_*` screening-call **payload builders** (regular + SPA) | ~3,260–3,810, 4,420+ | pure but interconnected | either | pure dict builders (`_ja_candidate_screening_call_payload`, `_ja_spa_screening_call_*`, `_ja_screening_call_answers`, answer-model variants) but they share `_onenote_clean_field_value` / `_onenote_screening_remarks_value` / `_onenote_presentability_rating_int` (also **injected** into `cvstudio_ja_salary_ai`). A characterization-tests-first project spanning ja_answers + onenote; the 12-variant answer-shape logic needs pinning first |
