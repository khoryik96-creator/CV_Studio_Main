...
function renderSummaryText(raw) {
  ...
  var src = (bulletLines.length ? bulletLines : lines).map(function(l){ return l.replace(/^[-•*]\s+/, '').trim(); }).filter(Boolean);
...
function initSummaryTabUI() {
  try {
    ...
    if (ta && !ta._summaryCharCountBound) {
      ta.addEventListener('input', updateSummaryCharCount);
