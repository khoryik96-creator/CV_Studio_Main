...
_INSTALL_RECEIPT_VERSION = "v24.6.398"
...
def cvstudio_download_folders():
    try:
        ...
        if data is None:
            ...
            return _cvstudio_error_payload(
                "DOWNLOAD_FOLDER_REQUEST_INVALID",
                "Download-folder settings must be a JSON object.",
                400,
            )
        kind = data.get("kind")
...
def jobadder_spider_options():
    ...
    if name in custom_field_options:
        ...
        try:
            ...
            error = "{}:{}".format(endpoint, e.code)
        except Exception as e:
            error = "{}:{}".format(endpoint, str(e)[:80])
        return jsonify({
            "items": fallback,
            "fallback": True,
            "field_id": field_id,
...
def jobadder_spider_search():
    ...
    try:
            ...
            for item in raw_items:
                ...
                if active_states and all(status in terminal_matches for status in active_states):
                    if cid:
                        ...
                        custom_filter_detail_map[cid] = item
                    continue
        ...
        excluded_count += detail_excluded_count + custom_filter_excluded_count
...
def parse_cv():
    try:
        ...
        parsed = _attach_untitled_subsidiary_entries(parsed, cv_text)
