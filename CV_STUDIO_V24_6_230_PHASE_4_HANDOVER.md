...
# CV Studio v24.6.230 → Phase 4 Handover

Source release: private owner/source v24.6.230

Completed phase: Phase 3 — shared external-service client foundations,
post-release review closure and JobAdder content-negotiation correction

Phase 3 source baseline: v24.6.222 at
`1be9da48d8307c418d82807cbdaedc9f876a1b15`

Previous completed release: v24.6.224 at
`0892dcc1fbec2fb68b4668014792230249c73cae`

## Activation gate

Phase 4 is not active. Start it only after an explicit owner instruction. Read
the v24.6.230 corrective QA report, the v24.6.224 corrective QA report, the
v24.6.223 Phase 3 QA report, this handover, the Phase 2B QA reports and the
historical Phase 2A reports. Verify the v24.6.230 owner/source archive against
...
- Keep all 107 Flask route URLs and every established success/failure response
  ...
  request across an authentication, CSRF or size boundary.
...
`cvstudio_clients.py` remains the external-service boundary. Future module moves
must retain its dependency-injected adapters and policies:

- `JobAdderClient.request_json` supplies `Accept: application/json` only when
  the caller has not supplied an Accept header. The selected header survives
  the one rejected-token refresh/retry.
- `JobAdderClient.request_raw` does not impose JSON content negotiation.
  Candidate-CV and attachment downloads remain representation-neutral unless
  their caller supplies an Accept header; any supplied header is honored
  case-insensitively.
- JobAdder exact download bytes/content metadata and activity-diagnostic legacy
  success/error fields remain unchanged.
- JobAdder safe reads retain bounded retry, one rejected-token refresh/retry,
  reconnect marking and bounded offset pagination. Mutations/uploads do not
  replay after ambiguous transient failure.
...
- `AIProviderClient` retains exact Anthropic, DeepSeek and OpenAI endpoints and
  headers, 15–300 second timeout bounds, HTTPS host restriction and zero
  retries for every chargeable POST.
- Every production redirect hop is constrained to the client's HTTPS host
  allowlist. Sensitive headers are stripped when an allowed redirect changes
  origin, and rejected redirect responses are closed before a structured error
  is raised.
- Successful response headers retain case-insensitive lookup. Keep DeepSeek
  web-tool refusal, OpenAI request/response translation, usage normalization,
  route-specific friendly failures and paid-call confirmation gates unchanged.
...
- The local watchdog, Tavily/SerpAPI search and Apollo enrichment remain outside
  the three Phase 3 clients. Do not silently expand client scope.
...
1. Verify the v24.6.230 owner ZIP and fresh extraction against the adjacent
   SHA-256 and verification sidecars.
2. Verify Git is clean and based on the completed v24.6.230 release commit
   recorded in the verification sidecar.
3. Read `AGENTS.md`, `ROADMAP.md`, `PHASE_STATUS.md`, `IMPLEMENT.md`, this
   handover, the v24.6.230 corrective QA report, the v24.6.224 corrective QA
   report, the v24.6.223 Phase 3 QA report and prior Phase 2A/2B reports.
4. Run all 48 Python tests with resource warnings treated as errors, both
...
7. Re-prove the exact 107-route inventory and all Phase 3 no-network client
   characterization fixtures, including redirect cleanup, response-header
   semantics and JobAdder raw/JSON content negotiation.
8. Inventory the first proposed module boundary and record the authorized Phase
   4 milestone plan before moving code.

## Native-test caveat

v24.6.230 has genuine Windows source execution and controlled local fixtures,
